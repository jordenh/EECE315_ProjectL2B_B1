...
int main(void){
    ...
    while(!exitBool){
		...
		tempChar = *(argv[0]+charParserIndex);
        while(tempChar != '$' ){
            if(nextCommand.argc = 0){
				if(tempChar != ' '){
					nextCommmand.name[individualCharIndex++] = nextCommand.argv[0][charParserIndex];
				}
				else{
                ...
                if(tempChar != ' '){
					nextCommmand.argv[nextCommand.argc][individualCharIndex++] = nextCommand.argv[0][charParserIndex];
				}
				else{
					nextCommand.argc++;  //move to next string
					individualCharIndex = 0;
